...
from backpage.models import Rental, Lessee, User
...
class NearLesseeTestCase(TestCase):
    ...
    def setUp(self):
        ...
        lessee.save()
        Lessee.objects.create(id=lessee, position_x=30.317379, position_y=120.343004, password="1", realname="1", ci="1")
        lessee = User(account="3", name="lessee2", user_type=2, token="1111")
        lessee.save()
        Lessee.objects.create(id=lessee, position_x=30.317715, position_y=120.342891, password="1", realname="1", ci="1")
        lessee = User(account="4", name="lessee3", user_type=2, token="1111")
        lessee.save()
        Lessee.objects.create(id=lessee, position_x=30.317734, position_y=120.342562, password="1", realname="1", ci="1")
        lessee = User(account="5", name="lessee4", user_type=2, token="1111")
        lessee.save()
        Lessee.objects.create(id=lessee, position_x=40.317379, position_y=110.343004, password="1", realname="1", ci="1")
    ...
    def test_get_near_lessees_list_success_with_query(self):
        response = self.client.get(
            "/rent/{}/near_lessees/?token=123&positoinx=30.317636&positiony=120.342755&limit=40".format(self.rental_id))
